...
This module provides the Game class which manages the game loop,
event handling, asset loading, and overall game flow.
"""
...
from constants import (
    ...
    RED, YELLOW, INITIAL_BLOCK_SPEED
)
from menu import GameMode, Difficulty, MenuScreen, create_fonts, DIFFICULTY_CONFIG
...
class Game:
    ...
    Manages the game loop, event handling, rendering, asset loading,
    ...
    def load_and_scale_image(self, image_path: str, new_height: int) -> pygame.Surface:
        ...
        Args:
            ...
            new_height: Desired height in pixels
    ...
    def handle_difficulty_input(self) -> bool:
        """Handle input in difficulty selection."""
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                return False
            if event.type == pygame.KEYDOWN:
    ...
    def render(self):
        """Render the current game screen based on mode."""
        if self.mode == GameMode.MENU:
            ...
            # Draw HUD
            UIRenderer.render_hud(self.screen, self.game_state, self.fonts)
